...
class DouyinPlugin extends PlatformInterface {
  constructor(config = {}) {
    super(config)
  }
  ...
  async checkLoginStatus(browserController) {
    try {
      ...
      console.error('[DouyinPlugin] 检测登录状态失败:', error)
      return false
    }
  }
  ...
  async publish(browserController, content, options = {}) {
    try {
      ...
      console.error('[DouyinPlugin] 发布失败:', error)
  ...
  async getAccountInfo(browserController) {
    try {
      const page = await browserController.getActivePage()
      await page.goto('https://www.douyin.com/user/self', { timeout: 30000 })

      const info = await page.evaluate(() => {
        return {
          username: document.querySelector('.username')?.textContent?.trim() || '',
          followers: document.querySelector('.followers')?.textContent?.trim() || '0'
        }
      })

      return info
    } catch (error) {
      console.error('[DouyinPlugin] 获取账号信息失败:', error)
      return null
    }
  }

  getRPAScriptPath() {
    return path.join(__dirname, '../../../resources/rpa-scripts/douyin.mjs')
  }

  getCookieDomains() {
    return ['.douyin.com', 'douyin.com']
  }
}

export default DouyinPlugin
