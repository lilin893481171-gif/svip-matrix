/**
 * @file xiaohongshu-adapter.js
 * 小红书适配器 (v30 重写) - 实现统一适配器接口
 */
...
export class XiaohongshuAdapter extends Adapter {
  ...
  async init(browser, page, config) {
    ...
    console.log('[XiaohongshuAdapter] 适配器初始化完成');
  }
  ...
  async navigateToPublish() {
    ...
    console.log('[XiaohongshuAdapter] 导航到小红书发布页面...');
    ...
    console.log('[XiaohongshuAdapter] 已到达发布页面');
  }
  ...
  async uploadAndCaptureId(videoPath) {
    ...
    console.log(`[XiaohongshuAdapter] 开始上传视频: ${videoPath}`);
    ...
    const videoIdPromise = new Promise((resolve, reject) => {
      ...
      this.page.on('response', async (response) => {
        ...
        if (url.includes('/api/media/v1/video/upload')) {
          try {
            const data = await response.json();
            if (data && data.data && data.data.video_file_id) {
              clearTimeout(timeout);
              resolve({ video_file_id: data.data.video_file_id });
            }
          } catch (e) {
            // 解析 JSON 失败，继续等待
          }
    ...
    console.log('[XiaohongshuAdapter] 视频文件已选择，等待上传完成...');

    // 等待并返回视频ID
    const result = await videoIdPromise;
    console.log(`[XiaohongshuAdapter] 视频上传完成，获得 video_file_id: ${result.video_file_id}`);
    return result;
  }
  ...
  async fillForm(task) {
    ...
    if (task.description) {
      ...
      console.log('[XiaohongshuAdapter] 正文填写完成');
    }

    // 处理标签/话题（后续迭代）
    if (task.tags && Array.isArray(task.tags)) {
      // 暂时跳过标签处理
      console.log('[XiaohongshuAdapter] 标签处理（待实现）');
    }

    // 处理定时发布（后续迭代）
    if (task.scheduleTime) {
      // 暂时跳过定时发布处理
      console.log('[XiaohongshuAdapter] 定时发布处理（待实现）');
    }

    console.log('[XiaohongshuAdapter] 表单填写完成');
  }
  ...
  async publish() {
    ...
    console.log('[XiaohongshuAdapter] 开始发布内容...');
    ...
    try {
      ...
      console.log('[XiaohongshuAdapter] 内容发布成功');
      return { success: true };
    } catch (error) {
      console.error('[XiaohongshuAdapter] 发布失败:', error);
   ...
   */
  async cleanup() {
    console.log('[XiaohongshuAdapter] 清理适配器资源...');
    this.browser = null;
    this.page = null;
    this.config = null;
    this.isConnected = false;
    console.log('[XiaohongshuAdapter] 适配器资源清理完成');
  }
}
